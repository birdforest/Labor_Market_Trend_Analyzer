...
def get_income_map(state_code, industry_code, reduction):
    ...
    if not all(col in df.columns for col in ["total_income", "population", "industry_wage", "industry_employment"]):
        return jsonify({"error": "Missing required columns in the DataFrame"}), 500
    ...
    return jsonify(geoPandas.to_json())
