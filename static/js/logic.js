...
function refreshIncomeMap(state_code, industry_code, reduction) {
  // Define URL for API
  const geoData = `http://127.0.0.1:5000/api/v1.0/get_income_map/${state_code}/${industry_code}/${reduction}`;
  ...
  d3.json(geoData).then(function (data) {
    ...
    // Focus map on a selected state or the whole mainland US
    if (state_code === "US") {
    ...
    incomeLegend.onAdd = function () {
      ...
      const limits = incomeChoroplethLayer.options.limits;
      const colors = incomeChoroplethLayer.options.colors; 
    ...
    console.error('Error fetching income map data:', error);
  });
}
